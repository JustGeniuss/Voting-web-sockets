...
export class PollsRepository {
  ...
  async addNomination({
    ...
    this.logger.log(
      `Attempting to add a monimation with nominationID/nomination: 
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Failed to add a nomination with nominationID/text:
        ${nominationID}/${nomination.text} to pollID: ${pollID}`,
      );
    }
  }
  ...
  async removeNomination(pollID: string, nominationID: string): Promise<Poll> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to remove a nomination with nominationID: ${nominationID}} from pollID: ${pollID}`,
      );

      throw new InternalServerErrorException(
        `Failed to remove a nomination with nominationID:
        ${nominationID} from pollID: ${pollID}`,
      );
    }
  }
  ...
  async addParticipantRankgins({
    pollID,
    userID,
    rankings,
  }: AddParticipantRankingsData): Promise<Poll> {
    this.logger.log(
      `Attempting to add rankgins for userID/name: ${userID} to pollID: ${pollID}`,
      rankings,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to add rankgins for userID/name: ${userID} to pollID: ${pollID}`,
        rankings,
      );
